...
class DocumentMetadata(BaseModel):
    ...
    def effective_file_content(self) -> Optional[str]:
        """Get file content with fallback chain"""
        return self.file_content or self.fileContent
    
    @property  
    def effective_file_path(self) -> Optional[str]:
        """Get file path with fallback chain"""
        return self.file_path or self.filePath
...
class OCRAgentV2:
    ...
    async def _get_file_bytes_robust(self, metadata: DocumentMetadata) -> bytes:
        ...
        # === ERROR CASE ===
        error_details = {
            "file_content_candidates": [bool(c) for c in file_content_candidates],
            "file_path_candidates": [(c, os.path.exists(c) if c else False) for c in file_path_candidates],
